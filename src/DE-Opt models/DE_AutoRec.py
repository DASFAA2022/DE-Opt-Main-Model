...
class DE_UAutoRec():
    def __init__(self, sess, num_user, num_item, learning_rate=0.001, reg_rate=0.01, epoch=500, batch_size=4096,
                 verbose=False, T=3, display_step=1000, individual_num=5, lr_min=0.0001, lr_max=0.01, reg_rate_min=0.001, reg_rate_max=0.1,
                 min_RMSE_DE=1e10, min_round=0,  delay_round=20, total_round=0):
    ...
    def GenerateTrainVector(self, ID, maxID, lr_matrix, reg_rate_matrix):
        ...
        temp1 = lr_matrix[u2] - lr_matrix[u3]
        temp2 = temp1 * F
        temp_mutation = lr_matrix[u1] + temp2
        temp1 = temp_mutation - lr_matrix[ID]
        temp2 = temp1 * K
        Result[0] = lr_matrix[ID] + temp2

        temp1 = reg_rate_matrix[u2] - reg_rate_matrix[u3]
        temp2 = temp1 * F
        temp_mutation = reg_rate_matrix[u1] + temp2
        temp1 = temp_mutation - lr_matrix[ID]
        temp2 = temp1 * K
        Result[1] = reg_rate_matrix[ID] + temp2
    ...
    def train(self, train_data):
        ...
        for i in range(total_batch):
            ...
            print('lr:%.4f' % self.learning_rate, 'reg:%.4f' % self.reg_rate)
            _, loss = self.sess.run([self.optimizer, self.loss],
                                    feed_dict={self.rating_matrix: self.train_data[:, batch_set_idx],
                                               self.rating_matrix_mask: self.train_data_mask[:, batch_set_idx]
                                               })
    ...
    def execute(self, train_data, validation_data, test_data):
        ...
        for epoch in range(self.epochs):
            time1 = time.time()
            rmse_no_de, mae_no_de = self.validation(validation_data)
            for ID in range(self.individual_num):
                evolution = self.GenerateTrainVector(ID, self.individual_num, self.lr_matrix, self.reg_rate_matrix)
                self.learning_rate = evolution[0]
                self.reg_rate = evolution[1]
                self.train(train_data)
                rmse_de, mae_de = self.validation(validation_data)
                if rmse_de <= rmse_no_de:
                    self.lr_matrix[ID][0] = evolution[0][0]
                    self.reg_rate_matrix[ID][0] = evolution[1][0]
                    rmse_no_de = rmse_de
